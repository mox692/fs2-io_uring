...
private[uring] final class ArrayStack[A <: AnyRef](
...
private[uring] object ArrayStack {

  def apply[A <: AnyRef](): ArrayStack[A] = new ArrayStack()

  def apply[A <: AnyRef](ptr: Ptr[iovec], size: Int): ArrayStack[A] = new ArrayStack(size)
