...
void fs2_io_uring_prep_read_fixed(struct io_uring_sqe *sqe, int fd, void *buf,
                              size_t len, __u64 offset, int index) {
  io_uring_prep_read_fixed(sqe, fd, buf, len, offset, index);
}

void fs2_io_uring_register_buffers(struct io_uring *ring, const struct iovec *iovecs, unsigned nr_iovecs) {
